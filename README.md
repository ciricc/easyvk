...
### Реальный пример работы Midleware'ов

Допустим, нам нужно добавить в библиотеку поддержку обратной совместимости с прошлыми версиями. Ну, представим, что плагин авторизации использует поле `response.fields.user_id` для получения идентификатора, но в новой версии `API` все имзенилось, и теперь это поле хранится в поле `response.user_id`. Давайте решим эту проблему, чтобы не переписывать весь код с нуля.

```javascript
const {VK} = require('easyvk');

// ПРОШУ ЗАМЕТИТЬ, ЧТО ЭТОТ КОД НЕ БУДЕТ РАБОТАТЬ  НА РЕАЛЬНОМ ПРИМЕРЕ!!
// ЭТА СИТУАЦИЯ МОЖЕТ ПРОИЗОЙТИ В ДРУГОМ ВИДЕ, НО РЕШЕНИЕ БУДЕТ ПОХОЖЕЕ

let vk = new VK();

vk.use('api.resolveApiRequest', async (context, next) => {
  
  // Если метод - users.get, то тогда делаем кастомизацию ответа сервера
  if (context.response.config.url.match(/method\/users.get/)) {
    // Проверяем, что ответ возвращен в новом формате (последней версии API)
    if (context.response.data.user_id) {
      context.response.data.response = { // Обновляем ответ сервера на нужный нам
        ...context.response.data.response, // Добавляем поддержку нового кода
        fields: { // Добавляем поддержку старого кода
          user_id: context.response.data.response.user_id
        }
      }
    }
  }
...
Иногда в вопросе обратной совместимости может возникнуть ситуация, когда изменяет не просто ответ сервера, а еще и путь, куда нужно отправлять запрос, чтобы получить ответ сервера. Мы можем исправить и это


```javascript
vk.use('api.prepareApiConfig', async (context, next) => {
  
  // Провряем, что обращения идут к домену vk.ru
  if (context.url.match(/api\.vk\.ru/)) {
    // Заменяем vk.ru на vk.com
    context.url = context.url.replace(/vk\.ru/g, "vk.com");
  }
...
Надеюсь с такимим примерами вам стало намного понятнее как пользоваться прослойками!